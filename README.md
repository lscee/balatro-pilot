...
![Dashboard 展示当前游戏、一键启停、组件健康、自学习数据与高频模型切换](docs/assets/screenshots/dashboard-control-health.png)

Dashboard 把当前游戏快照、AI 一键暂停/启动、组件健康、自学习知识库、战绩与事件索引集中在一个页面，并可在本地 Qwen 与 DeepSeek 高频后端之间切换。图中正处于“一键启动”的恢复阶段，因此 Balatro、BalatroBot、控制器和直播叠层暂时显示离线；不同颜色明确区分“正常、已配置、启动中与离线”，便于判断系统是在等待依赖，还是发生了真实故障。健康检查只读取本地进程、端口和文件状态，不会因为页面刷新产生付费模型请求。
